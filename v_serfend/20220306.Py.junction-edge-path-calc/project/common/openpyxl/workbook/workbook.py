...
class Workbook(object):
    ...
    def _setup_styles(self):
        ...
        self.add_named_style(NamedStyle(
    ...
    def exist(self, sheet_name):
        return sheet_name in self._sheets_dict

    def get_sheet_with_create(self, title=None, index=None, put_index=None):
        ...
        if not title is None:
            return (self[title], False) if self.exist(title) else (self.create_sheet(title, put_index), True)
        if not index is None:
            if index >= 0 and index < len(self.sheets):
                return (self.sheets[index], False)
        s_name = f'new_sheet_{len(self.sheets)}'
        return (self.create_sheet(s_name, put_index), True)
    ...
    def sheets(self, v):
        self._sheets = v
        self._sheets_dict = {}
        for x in self._sheets:
            self._sheets_dict[x.title] = x
